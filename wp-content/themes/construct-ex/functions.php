...
function construct_setup() {
	...
	add_image_size( 'construct-large-thumb', 830 );
	add_image_size( 'construct-medium-thumb', 550, 400, true );
	add_image_size( 'construct-small-thumb', 230 );
	
	//Register nav menu used in theme.
	register_nav_menus( array(
		'primary' 	=> __( 'Primary Menu', 'construct-ex' )
	) );
...
/*--- Blog layout ---*/
function construct_blog_layout() {
...
function construct_posted_on() {
	$layout = construct_blog_layout();
...
function construct_excerpt_more($more) {
	global $post;
	$layout = construct_blog_layout();
	return ' ....&nbsp;&nbsp;<a class=" '.$layout.'" href="'. get_permalink($post->ID) . '">' . __('Read More', 'construct-ex') . '</a>';
}
add_filter('excerpt_more', 'construct_excerpt_more');
...
function construct_scripts() {
	...
	wp_enqueue_script( 'construct-common', get_template_directory_uri() . '/js/common.js', array('jquery'), '20180213', true );
	...
	wp_enqueue_style( 'construct-font-awesome', get_template_directory_uri() . '/fonts/font-awesome.min.css' );
...
// Enqueue Bootstrap
function construct_enqueue_bootstrap() {
	wp_enqueue_style( 'construct-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true );
}
add_action( 'wp_enqueue_scripts', 'construct_enqueue_bootstrap', 9 );
...
function construct_widgets_init() {
    register_sidebar( array(
        'name' => __( 'Main Sidebar', 'construct-ex' ),
        'id' => 'sidebar-1',
        'description' => __( 'Widgets in this area will be shown on all posts and pages.', 'construct-ex' ),
	...
	for ($i=1; $i<=3; $i++){
		register_sidebar( array(
			'name' => __( 'Footer-widget-area-', 'construct-ex' ) . $i,
			'id' => 'footer-widget-area' . $i,
			'description' => __( 'Widgets in this area will be shown on footer.', 'construct-ex' ),
			'before_widget' => '<li id="%1$s" class="widget %2$s">',
			'after_widget'  => '</li>',
			'before_title'  => '<h3 class="widgettitle">',
			'after_title'   => '</h3>',
...
function construct_edit_link() {
	edit_post_link(
		sprintf( __( '<i class="fa fa-edit"></i>Edit<span class="screen-reader-text"> "%s"</span>', 'construct-ex' ), get_the_title() ),
...
function construct_get_breadcrumb() {
    echo '<a href="'.home_url().'" rel="nofollow">'.__('Home', 'construct-ex').'</a>';
    if ( is_category() || is_single() ) {
        ...
        echo __("&nbsp;&nbsp;&#47;&nbsp;&nbsp;Search Results for... ", "construct");
        echo '"<em>';
...
function construct_recommended_plugin() {
...
add_action( 'tgmpa_register', 'construct_recommended_plugin' );
...
add_filter( 'pre_set_site_transient_update_themes', 'construct_check_for_update' );
function construct_check_for_update( $checked_data ){
...
popularfx_maybe_promo([
	'after' => 1,// In days
	'interval' => 10,// In days
	'pro_url' => 'https://www.popularfx.com/themes/wordpress/corporate/Construct%20Pro',
	'rating' => 'https://popularfx.com/themes/wordpress/corporate/Construct-Ex',
